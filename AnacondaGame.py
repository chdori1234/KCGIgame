...
def game():
    ...
    class Feed(object):
        ...
        def create(self):
            self.position = (
            random.randint(0, SHELL_WIDTH - 1) * SHELL_SIZE, random.randint(0, SHELL_HEIGTH - 1) * SHELL_SIZE)
    ...
    def GameOver(surface):
        font = pygame.font.Font(None, 34)
        tip = font.render('Press the Q or ESC to exit the game', True, (65, 105, 225))
        surface.blit(tip, (80, 280))
        ...
        while True:
            for event in pygame.event.get():  # event handling loop
                if event.type == QUIT:
                    ...
                    if event.key == K_ESCAPE or event.key == K_q:  # プログラムを終了
                        pygame.quit()
                        sys.exit()
                    else:
                        return
...
users=[{"username":"123456","password":"000000"}] 
user={"username":"","password":""}
new_user={"username":"","password":""}

def login():
    username = input("アカウントを入力してください　:")
    ...
    if user in users:
        ...
        print("アカウント存在しません、新しく作成してください")
            
        new_username = input("新しいアカウントを入力してください　:")
        ...
        if new_username == "" and new_password == "":
            ...
            users.append(user)
            print("新規登録しました、もう一度ログインしてください")
            u = input(" アカウントを入力してください　:")
